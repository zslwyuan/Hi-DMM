...
static ap_uint<64> group_tree[70]={ 0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,
...
static ap_uint<8> addr_layer_map[2048];
...
volatile void HTA2048_theta(volatile allocator_port *alloc)//, volatile ap_uint<64> //  --*port1, volatile ap_uint<64> //  --*port2)
{
				  ...
				  #pragma HLS protocol fixed
					...
					if (tmp)
					{
						if (ans>7) //fine-grained
						{
							...
							TMP_1 = TMP_0&(~(TMP_0-1));
							loc_tree = log_2_64bit(TMP_1);  // find the first set bit in the layer of group tree
							new_loc1 = loc_tree+(loc1<<(ans-7))-shift_constant[ans-8];//get the offset in the deep layer
							alloc->addr = output_addr = (new_loc1)<<(12-ans);
						...
						{
							alloc->addr = output_addr = (loc1)<<(12-ans);
...
#pragma HLS pipeline
							...
							buddy_tree[12]=buddy_tree[12]&(tmp);
					}
					else
					{
...
#pragma HLS UNROLL factor=2
							...
							buddy_tree[12].set_bit(loc_tree,(bool)((TMP_0&0x3FFFFFFFC0000000)!=0));
					}
				}
			}
				...
				io_section8:{
				  #pragma HLS protocol fixed
					...
					if (ans<=7)// coarse-grained maintenance
					{
							...
							//  --*port1 = 0xbb1;
							//  --*port2 = loc1;
							buddy_tree[ans].set(loc1,1);		// free the node
							tmp=(1<<loc1);
							loc2 = loc1+1;
