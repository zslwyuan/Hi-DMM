...
static ap_uint<4> group_tree[70]={ 0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,//HTA1024
...
static ap_uint<8> addr_layer_map[128];//HTA1024
static ap_uint<8> addr_tree_map[128];//HTA1024
...
static ap_uint<4> mark_mask[32]={0xD, 0x2, 0x5, 0x9, 0x2, 0x2, 0x5, 0x5,
		0x9, 0x9, 0x2, 0x2, 0x2, 0x2,

		0xD, 0x2, 0x4, 0x8, 0x0, 0x0, 0x0, 0x0,
		0x0, 0x0, 0x0, 0x0, 0x0, 0x0
};


volatile void HTA128_theta(volatile allocator_port *alloc)//, volatile ap_uint<64>  //  --*port1, volatile ap_uint<64>  //  --*port2)
{
				  ...
				  #pragma HLS protocol fixed
					...
					if (tmp)
					{
						if (ans>7) //fine-grained
						{
							...
							alloc->addr = output_addr = (new_loc1)<<(8-ans);
						...
						{
							alloc->addr = output_addr = (loc1)<<(8-ans);
					...
					//  --*port1 = 0xe2;
					//  --*port2 = addr_layer_map[output_addr];
					//  --*port1 = 0xe3;
					//  --*port2 = addr_tree_map[output_addr];
					now2 = ans;
					now1 = ans;
			...
			else if (cmd == 3)  // executing the free request
			{
  			...
  			/*				for (now1 = ans-1, loc1>>=1, now2 = ans+1, loc2=((loc2-1)<<1), cnt=1; now1<=7||now2>=1; now1+=(now1!=8),now2-=(now2!=0))   //upward maintenance
					...
					else
					{
							...
							//  --*port1 = 0xbb2;
							//  --*port2 = loc1;
							loc_tree = (free_target>>(8-ans))-(loc1<<(ans-7))+shift_constant[ans-8];//which node in the layer
							//  --*port1 = 0xbb3;
							//  --*port2 = loc_tree;
