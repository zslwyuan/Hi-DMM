...
static ap_uint<64> heap_tree[64]={0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,
								...
								0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,
								0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF};
static ap_uint<32> used_free[4096];
...
volatile void KWTA_mini16_theta(volatile allocator_port *alloc)//, volatile ap_uint<32> *port1, volatile ap_uint<32> *port2)
{
...
#pragma HLS protocol fixed
					...
					else if (last_offset(19,19)==0) //try to pre-allocate //miniheap
					{
						io_section5:{
						  #pragma HLS protocol fixed
							alloc->addr = last_addr;   // if output_addr<0, it means the allocation turns out to be a failure.
						}
						last_addr++;
						cur_used=used_free[last_offset].range(9,5);
						if (cur_used==15)   //miniheap
						{
							//heap_tree[last_loc1]=heap_tree[last_loc1]&(~(1<<(last_loc2-1)));
							heap_tree[last_loc1].set(last_loc2,0);
							last_offset=-1;
							if (heap_tree[last_loc1]==0)
							{
							//	top_heap = top_heap&(~(1<<(last_loc1-1)));
								top_heap.set(last_loc1,0);
							}
						}
						else used_free[last_offset].range(9,5)=cur_used+1;
					}
					else // do allocation when pre-allocation fails to meet the request
					{
						last_loc1=loc1 = log_2_64bit((ap_uint<64>)(top_heap-(top_heap&(top_heap-1))));
						last_loc2=loc2 = log_2_64bit((ap_uint<64>)(heap_tree[loc1]-(heap_tree[loc1]&(heap_tree[loc1]-1))));
						last_offset = offset = (loc1<<6)+loc2;
						used_free[offset].range(9,5) = 1;
						last_addr = (offset<<4)+1;//miniheap
						io_section55:{
						  #pragma HLS protocol fixed
							alloc->addr = last_addr-1;   // if output_addr<0, it means the allocation turns out to be a failure.
						}
			...
			else if (cmd == 3)  // executing the free request
			{
				...
				tmp_loc2 = free_target(9,4);//miniheap
				tmp_loc1 = free_target(20,10);//miniheap
				tmp_offset = free_target(20,4);//miniheap
				cur_free = used_free[tmp_offset].range(4,0);
				if (cur_free==15)//miniheap
				{
					...
					used_free[tmp_offset].range(4,0) = cur_free + 1;
			}
			else
			{
				io_section6:{
