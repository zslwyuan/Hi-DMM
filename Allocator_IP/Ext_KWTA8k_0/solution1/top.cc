...
static ap_uint<32> heap_tree[128]=	{	0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,
...
static ap_uint<8> mark_mask[128]={0x3FFFC03FC3CD,0x3FFFC0003FC03C32,0x3FC003C0C5,0x3FC0003C0309,0x3FC00003C00C12,0x3FC000003C003022,
...
static ap_uint<8> group_tree[2048];

static ap_uint<64> top_heap[2]={0xFFFFFFFFFFFFFFFF,0xFFFFFFFFFFFFFFFF};
...
volatile void Ext_KWTA8k(volatile allocator_port *alloc,volatile KTWA2HTA_port *com_port)//, volatile ap_uint<32> *port1, volatile ap_uint<64> *port2)
{
#pragma HLS ARRAY_PARTITION variable=top_heap complete dim=1
#pragma HLS ARRAY_PARTITION variable=heap_tree block factor=2 dim=1
...
#pragma HLS protocol fixed
	...
	//				*port2 = loc_in_layer;
					io_section55:{
						alloc->addr = (loc_in_layer<<(1-layer1));   // if output_addr<0, it means the allocation turns out to be a failure.
					}
					io_section_help2:
					{
//#pragma HLS protocol fixed
...
#pragma HLS protocol fixed
	...
	//					*port2=top_heap[1];
				...
				else
				{
					layer-=12;
					addr_HTA = free_target>>2;
					free_target= (free_target>>(1-layer));
	//				*port1 = 0xB;
	//				*port2 = free_target;
