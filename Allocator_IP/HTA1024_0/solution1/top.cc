...
static ap_uint<32> group_tree[70]={ 0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF,//HTA1024
...
static ap_uint<8> addr_layer_map[1024];//HTA1024
static ap_uint<8> addr_tree_map[1024];//HTA1024
...
static ap_uint<32> mark_mask[128]={0xC03FC3CD, 0x3FC03C32, 0xC003C0C5, 0x3C0309, 0x3C00C12, 0x3C003022, 0xC000C045, 0x30085,//HTA1024
		0xC0109, 0x300209, 0xC00412, 0x3000812, 0xC001022, 0x30002022, 0xC0004045, 0x8045,
		0x10085, 0x20085, 0x40109, 0x80109, 0x100209, 0x200209, 0x400412, 0x800412,
		0x1000812, 0x2000812, 0x4001022, 0x8001022, 0x10002022, 0x20002022, 0x40004045, 0x80004045,
		0x8045, 0x8045, 0x10085, 0x10085, 0x20085, 0x20085, 0x40109, 0x40109,
		0x80109, 0x80109, 0x100209, 0x100209, 0x200209, 0x200209, 0x400412, 0x400412,
		0x800412, 0x800412, 0x1000812, 0x1000812, 0x2000812, 0x2000812, 0x4001022, 0x4001022,
		0x8001022, 0x8001022, 0x10002022, 0x10002022, 0x20002022, 0x20002022,

		0xC03FC3CD, 0x3FC03C32, 0xC003C0C4, 0x3C0308, 0x3C00C10, 0x3C003020, 0xC000C040, 0x30080,
		0xC0100, 0x300200, 0xC00400, 0x3000800, 0xC001000, 0x30002000, 0xC0004000, 0x8000,
...
volatile void HTA1024_theta(volatile allocator_port *alloc)//, volatile ap_uint<64> // *port1, volatile ap_uint<64> // *port2)
{
				  ...
				  #pragma HLS protocol fixed
					...
					if (tmp)
					{
						if (ans>7) //fine-grained
						{
							...
							alloc->addr = output_addr = (new_loc1)<<(11-ans);
						...
						{
							alloc->addr = output_addr = (loc1)<<(11-ans);
...
#pragma HLS pipeline
							...
							buddy_tree[11]=buddy_tree[11]&(TMP_0);
					}
					else
					{
...
#pragma HLS UNROLL factor=2
							...
							buddy_tree[11].set_bit(loc_tree,(bool)((tmp&0x3FFFC000)!=0));
					}
				}
			}
			else if (cmd == 3)  // executing the free request
			{
  			...
  			/*				for (now1 = ans-1, loc1>>=1, now2 = ans+1, loc2=((loc2-1)<<1), cnt=1; now1<=7||now2>=1; now1+=(now1!=8),now2-=(now2!=0))   //upward maintenance
					...
					else
					{
							...
							//  --*port1 = 0xbb2;
							//  --*port2 = loc1;
							loc_tree = (free_target>>(11-ans))-(loc1<<(ans-7))+shift_constant[ans-8];//which node in the layer
							//  --*port1 = 0xbb3;
							//  --*port2 = loc_tree;
