...
volatile void KWTA_mini4_theta(volatile allocator_port *alloc)//, volatile ap_uint<32> *port1, volatile ap_uint<32> *port2)
{
...
#pragma HLS protocol fixed
			...
			else if (cmd == 3)  // executing the free request
			{
				...
				tmp_loc2 = free_target(7,2);//miniheap
				tmp_loc1 = free_target(15,8);//miniheap
				tmp_offset = free_target(15,2);//miniheap
				cur_free = used_free[tmp_offset].range(4,0);
				if (cur_free==3)//miniheap
				{
