...
static ap_uint<8> group_tree[70]={ 0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,//HTA1024
...
static ap_uint<8> addr_layer_map[256];//HTA1024
static ap_uint<8> addr_tree_map[256];//HTA1024
...
static ap_uint<16> mark_mask[32]={0xCD, 0x32, 0xC5, 0x09, 0x12, 0x22, 0x45, 0x85,
		0x09, 0x09, 0x12, 0x12, 0x22, 0x22,

		0xCD, 0x32, 0xC4, 0x08, 0x10, 0x20, 0x40, 0x80,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00
};


volatile void HTA256_theta(volatile allocator_port *alloc)//, volatile ap_uint<64> // *port1, volatile ap_uint<64> // *port2)
{
				  ...
				  #pragma HLS protocol fixed
					...
					if (tmp)
					{
						if (ans>7) //fine-grained
						{
							...
							alloc->addr = output_addr = (new_loc1)<<(9-ans);
						...
						{
							alloc->addr = output_addr = (loc1)<<(9-ans);
			...
			else if (cmd == 3)  // executing the free request
			{
  			...
  			/*				for (now1 = ans-1, loc1>>=1, now2 = ans+1, loc2=((loc2-1)<<1), cnt=1; now1<=7||now2>=1; now1+=(now1!=8),now2-=(now2!=0))   //upward maintenance
					...
					else
					{
							...
							//  --*port1 = 0xbb2;
							//  --*port2 = loc1;
							loc_tree = (free_target>>(9-ans))-(loc1<<(ans-7))+shift_constant[ans-8];//which node in the layer
							//  --*port1 = 0xbb3;
							//  --*port2 = loc_tree;
