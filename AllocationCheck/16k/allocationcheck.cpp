...
int req_list[REQ_N]={3,12,2,1,1,1,8,3,3,13,13,15,7,5,11,7,16,16,1,1,3,3,5,5,2,5,4,1,14,4,3, 16,16,16,5,5,5, 7, 9,13, 16, 3, 13, 12, 4, 9};
int req_cmd[REQ_N] ={2, 2, 2,2,2,2,2,2,2, 2, 2, 2, 2,2,  2,  2, 2, 2,2,2,  2,  2,  2,  2,  2,  2,2,3, 3, 3, 2,  2, 2, 2, 2, 2, 2, 3, 3, 3,  3, 2,  2,  2,  2,   2};
int layer[64*1024];
    ...
    for (i=0;i<generated_num;i++)
    {
    	...
    	else if (cmd==0) 
    	{
    		...
    		if (cmd==2) 
    		{
    		    if (rand()%2)
    		    {
    		        req_list[i] = (1<<(rand()%10+4))+rand()%128;
    		    }
    		    else
    		    {
        		    req_list[i] = (1<<(rand()%4))+rand()%7;
    		    }
    		}
    		else 
    		{
