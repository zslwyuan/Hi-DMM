...
void HLS_SPFA_KWTA(int n, int map[20000], int dis_output[200],volatile allocator_port *alloc_HTA,volatile allocator_port *alloc_KWTA)//, volatile ap_uint<32> *port1, volatile ap_uint<32> *port2)
{
#pragma HLS RESOURCE variable=map core=ROM_2P_BRAM
#pragma HLS interface ap_hs port=alloc_HTA
#pragma HLS interface ap_hs port=alloc_KWTA
	int HTA_heap[4000];
	int *KWTA_heap = HTA_heap + 2000;
...
//#pragma HLS interface ap_vld port=port2
    ...
    offset_local_dis = HLS_malloc<1>(n,alloc_HTA);
    ap_wait();
   // return;
    size_local_dis = n;
    int *local_dis = HTA_heap + (offset_local_dis&(0xFFFFE));
    ...
    offset_head = HLS_malloc<1>(1,alloc_KWTA)*LIST_SIZE;
    int *head,*tail;
    head = KWTA_heap + offset_head;
    head[LIST_VAL] = 0;
    head[LIST_NEXT] = -1;
    offset_tail = offset_head;
    tail = KWTA_heap + offset_tail;//HLS has BUG for directly tail = head;
...
//    		*port2 = local_dis[i1];
            ...
            if (update0&&update1)
            {
                ...
                offset_new_node0 = HLS_malloc<1>(1,alloc_KWTA)*LIST_SIZE;
                new_node0 = KWTA_heap + offset_new_node0;
...
//    			*port2 = tail-HTA_heap;
                ...
                offset_new_node1 = HLS_malloc<1>(1,alloc_KWTA)*LIST_SIZE;
                new_node1 = KWTA_heap + offset_new_node1;
...
//    		*port2 = local_dis[i1];
        ...
        next = KWTA_heap + offset_next;
        HLS_free<1>(offset_head/LIST_SIZE,1,alloc_KWTA);
        offset_head = -1;
