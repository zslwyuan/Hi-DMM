...
def getTypeWidth(type_name):
    ...
    if (len(re.findall(r'[0-9]+', type_name)) >= 1):
        ...
        if (type_name.find("long") >= 0
                or type_name.find("double") >= 0):
            width = 64
        else:
            if (type_name.find("int") >= 0
                    or type_name.find("float") >= 0):
                width = 32
            else:
...
def main():
    ...
    if (top_function_line.find('()') < 0):
        top_function_line += ','
    top_function_line += " hidmm_alloc_port *" + allocator[0]
    insert_before.append({
        'line':
        top_function_node.location.line + 2,
        'string':
        "#pragma HLS interface ap_hs port=" + allocator[0]
    })
    for i in range(len(allocator) - 1):
        top_function_line += ", hidmm_alloc_port *" + allocator[i + 1]
        insert_before.append({
            'line':
            top_function_node.location.line + 2,
            'string':
            "#pragma HLS interface ap_hs port=" + allocator[i + 1]
        })
    ...
    for node in pointer_list:
        for struct_var in struct_list:
            if (node.type.get_pointee().spelling == struct_var['struct']
                ...
                struct_field_type_map[node.type.get_pointee().spelling] = field_type
                replacement.append({
                    'line':
                    node.location.line,
                    'string':
    ...
    print(
        "\n========== Pointers with Allocation  ==============",
        file=logfile)
    print("Please make sure that the information required by the following pointers has been included in the file of user define constrain (Priority and MAU size).")
    print("------------------------------------")
    tmp_string = ""
    for node in pointers_with_alloc:
        print(
            ...
            file=logfile)
        tmp_string = tmp_string +node.spelling+" "+node.type.spelling.replace(" *","").replace(" ","")+" "+str(getTypeWidth(node.type.get_pointee().spelling))+"\n"
        pointer_width_map[node.spelling]=getTypeWidth(node.type.get_pointee().spelling)
    ...
    print(tmp_string+"------------------------\n")
    print("Otherwise, MAU size of each of them will be 1 and the priority of them will be equal.")
    print("if necessary, you can indicate the user defined list by inserting an argument --user_define_list=xxx.")

    tot_priority = 0
    if (opts.user_define_list!=""):
        print("User indicates user define list. MAU size and the priority will be set according to the file "+opts.user_define_list+".")
        ...
        print("User indicates no user define list. MAU size will be 1 and the priority will be equal.")
        for node in pointers_with_alloc:
    ...
    for node in static_arrays_dict:
        ...
        partition_line = find_in_block(
            file_content, "#pragma HLS ARRAY_PARTITION", True, True)
        if (partition_line >= 0):
            if (extractVar(file_content[partition_line]) == node['name']):
                partition_factor_for_the_array = extractFactor(
                    file_content[partition_line])
    ...
    print(str(len(type_pointers.keys()))+" "+str(len(pointer_list)),file = mtx_file)
    print("\n",file = mtx_file)
    ...
    for key in type_pointers.keys():
            ...
            for nodeA in tmp_list:
                print( [pointer_co_matrix[pointer_id_map[str(get_cursor_id(nodeA.get_definition()))]][pointer_id_map[str(get_cursor_id(nodeB.get_definition()))]] for nodeB in tmp_list],     file=mtx_file) 
            print(" ",file = mtx_file)
    ...
    for attempt in range(0,attempt_num):
        ...
        for group in range(0,group_num):
            ...
            tmp_heap_comp_list = []
            var_name_list = [str(node.spelling) for node in type_pointers[type_list[var_type]] ]
            ...
            if (MAU_size_tmp==1000000):
                MAU_size_tmp = 1
            tmp_dict = {'var_type':var_type,'var_type_string':type_list[var_type],'comp_num':comp_num,'tmp_heap_comp_list':tmp_heap_comp_list,'tmp_heap_comp_list_string':tmp_heap_comp_list_string,'priority_accumulate':priority_accumulate,'MAU_size':MAU_size_tmp,'allocated_BRAM':int(BRAM_for_hidmm/tot_priority*priority_accumulate) }
            heap_in_attempt.append(tmp_dict)
        heap_attempts.append({'group_num':group_num,'heap_in_attempt':heap_in_attempt})
    ...
    for i in range(len(heap)):
        insert_before.append({
            ...
            1,
            'string':
            heap_type[i] + " " + heap[i] + "[" + str(heap_size[i]) + "];\n"
        })
