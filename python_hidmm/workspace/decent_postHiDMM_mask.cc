...
template <int unused> volatile int HLS_malloc(int size, volatile allocator_port *allocator)
{
#pragma HLS INLINE
	int status;
...
template <int unused> volatile int HLS_free(int free_target, int free_size, volatile allocator_port *allocator)
{
#pragma HLS INLINE
	int status;
io_section_HLS_free:
...
void top(int nouse, int just, int a_test, hidmm_alloc_port *Hi_DMM_allocator_0_Super_HTA32k, hidmm_alloc_port *Hi_DMM_allocator_9_Super_HTA64k, hidmm_alloc_port *Hi_DMM_allocator_2_KWTA,
		 hidmm_alloc_port *Hi_DMM_allocator_3_KWTA, hidmm_alloc_port *Hi_DMM_allocator_4_Super_HTA64k, hidmm_alloc_port *Hi_DMM_allocator_5_Super_HTA64k, hidmm_alloc_port *Hi_DMM_allocator_6_Super_HTA64k,
		 hidmm_alloc_port *Hi_DMM_allocator_7_Super_HTA64k, hidmm_alloc_port *Hi_DMM_allocator_8_Super_HTA64k)
{
...
#pragma HLS ARRAY_PARTITION variable = oo cyclic factor = 7
	...
	for (a = 1; a < 6; a++)
	{
		...
		offset_struct_tmp = HLS_malloc<163840>(1, Hi_DMM_allocator_2_KWTA);
		struct_tmp = Hi_DMM_dynamic_heap_2 + offset_struct_tmp; // HI-DMM insert: stress offset of pointer struct_tmp
		...
		offset_struct_tmp11 = HLS_malloc<163840>(1, Hi_DMM_allocator_2_KWTA);
		...
		offset_struct0_tmp = HLS_malloc<65536>(1, Hi_DMM_allocator_3_KWTA);
